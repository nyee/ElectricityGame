...
The ratio of the additive to reactor 1 stream mass flow rates is stream2Mass.
	...
	--Reactor_2_Aromatic_libraries on or off --Reactor_2_seed_with_aromatics on or off --Reactor_2_aromatic_seed on or off
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Reactor_2_aromatic_seed', type=str, help='Specify whether to use aromatic Libraries as seeds in Reactor 2 (Cracker)', default='off')
...
########################Write out correct reaction stream###############################################################
    ...
    speciesDict=loadSpeciesDictionary(os.path.join(args.oldWorkDir, "chemkin/species_dictionary_new.txt"))
    ...
    for item in dataList:
        ...
        if item.species is not None or item.label in bathGases:
            if item.data[-1]>tol:
	        if item.label.replace("(","_").replace(")","_") not in speciesDict:
                    continue
		elif speciesDict[item.label.replace("(", "_").replace(")", "_")].molecule[0].toSMILES() == "O=O":
                    continue
                else:
                    stream1[speciesDict[item.label.replace("(", "_").replace(")", "_")]]=item.data[-1]
	...
	seeded_aromatic_SMILES = ['C1=Cc2ccccc2C1', 'c1ccc2ccccc2c1']
...
####################Copy in, edit, write out new input file############################################################
    ...
    for line in inputList:
	    ...
	    if re.search('on', args.Reactor_2_Aromatic_libraries):
            	newLine=re.sub('\]', ",'" +args.name+ "', 'C10H11', 'C3', 'Fulvene_H', 'naphthalene_H', 'vinylCPD_H']", line)
	...
	#change the reaction libraries
	    ...
	    #Add aromatic libraries in addition to burner libraries if specified by user
	    if re.search('on', args.Reactor_2_Aromatic_libraries):
            	newLine=re.sub('\]', ", ('" +args.name+ "',True), ('phenyl_diacetylene_benzofulvenyl_effective',True), ('C10H11',True), ('C3',True), ('Fulvene_H',True), ('naphthalene_H',True), ('vinylCPD_H',True), ('biCPD_H_shift',True), ('2016_Mebel_C6H4C2H_C2H2_High_P',True), ('2006_Park_Phenyl_Propene',True), ('2012_Kislov_Phenyl_Propene',True), ('2016_Mebel_C9H9',True), ('2005_Ismail_C6H5_C4H6',True), ('2016_Mebel_Indene_CH3',True), ('2015_Wang_K_C6H9',True), ('2015_Buras_Vinyl_1_3_Butadiene',True), ('New_Phenyl_Propene_Pathway',True), ('Methyl_CPD_H_abs',True), ('C3H3_C7H7_Matsugi',True), ('C6H5_C4H4_all_TST_rates',True), ('c-C5H5_CH3_Sharma_TST',True), ('C10H9_Mebel_TST',True), ('C10H8_HACA',True), ('kislovB',True)]", line)
            	newInputList.append(newLine)
	    else:
		newLine=re.sub('\[', "[('" +args.name+ "',True), ('phenyl_diacetylene_benzofulvenyl_effective',False),", line)
            	newInputList.append(newLine)
        
        #change the seed mechanisms
        elif re.search('seedMechanisms', line):
	    #Add aromatic libraries in addition to burner libraries as seed mechanisms if specified by user
	    if re.search('on', args.Reactor_2_aromatic_seed):
            	newLine=re.sub('\]', ", 'phenyl_diacetylene_benzofulvenyl_effective', 'C10H11', 'C3', 'Fulvene_H', 'naphthalene_H', 'vinylCPD_H', 'biCPD_H_shift','2016_Mebel_C6H4C2H_C2H2_High_P','2006_Park_Phenyl_Propene','2012_Kislov_Phenyl_Propene','2016_Mebel_C9H9','2005_Ismail_C6H5_C4H6','2016_Mebel_Indene_CH3','2015_Wang_K_C6H9','2015_Buras_Vinyl_1_3_Butadiene','New_Phenyl_Propene_Pathway','Methyl_CPD_H_abs','C3H3_C7H7_Matsugi','C6H5_C4H4_all_TST_rates','c-C5H5_CH3_Sharma_TST','C10H9_Mebel_TST','kislovB','C10H8_HACA']", line)
            	newInputList.append(newLine)
	    else:
	...
	#Forbid singlet O2
            ...
            if not re.search('False', line): #not re.search('True', line):
                newLine=re.sub('True', "False", line)
                newInputList.append(newLine)
	    else:
		newInputList.append(line)
	...
	#Don't copy P-dep settings to Reactor 2 if user specifies P_dep to be off for that reactor
	    ...
	    dontCopy=True
	elif re.search("maximumAtoms", line) and re.search('on', args.Reactor_2_P_dep):    
	    newInputList.append("    maximumAtoms=12,\n")
	elif re.search("toleranceMoveToCore", line):
            newInputList.append("    toleranceMoveToCore=" + str(args.Reactor_2_tolmovetocore) + ",\n")
	elif re.search("toleranceKeepInEdge", line):
            newInputList.append("    toleranceKeepInEdge=" + str(0.1*args.Reactor_2_tolmovetocore) + ",\n")
	elif re.search("saveSimulationProfiles", line):
            newInputList.append("    saveSimulationProfiles=False,\n")
	else:
